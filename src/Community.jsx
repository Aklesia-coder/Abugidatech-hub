...
function Community() {
  ...
  if (loading) {
    return (
      <div className="flex h-screen w-full max-w-full bg-white dark:bg-[#0a0a14] text-gray-900 dark:text-white overflow-hidden">
        <Sidebar />
  ...
  return (
    <div className="flex h-screen h-[100dvh] w-full max-w-full overflow-hidden bg-white dark:bg-[#0a0a14] text-gray-900 dark:text-white transition-colors duration-300 relative">
      ...
      {/* Main Chat Column: flex-1 min-w-0 ensures no horizontal expansion */}
      <div className="flex-1 min-w-0 w-full max-w-full flex flex-col h-full overflow-hidden pb-14 md:pb-0 relative bg-white dark:bg-[#0a0a14]">
        ...
        <header
          ...
          className="cursor-pointer flex items-center justify-between gap-3 px-4 sm:px-6 py-3 border-b border-gray-200 dark:border-gray-800 bg-white/95 dark:bg-[#0a0a14]/95 backdrop-blur-sm z-10 shrink-0 select-none"
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            {!isMember ? (
              <button
                ...
                className="bg-purple-600 hover:bg-purple-700 text-white text-xs sm:text-sm font-semibold px-3.5 py-1.5 rounded-lg transition"
              >
                Join Group
              </button>
            ) : (
        ...
        {/* Messages Feed Container: overflow-x-hidden & min-w-0 prevents mobile right-gap */}
        <div className="flex-1 min-w-0 w-full max-w-full overflow-y-auto overflow-x-hidden px-3 sm:px-6 py-4 flex flex-col gap-3.5">
          {posts.length === 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    className={`max-w-[90%] sm:max-w-[75%] rounded-2xl overflow-hidden cursor-pointer shadow-xs transition-shadow min-w-0 ${
                      isMe
                  ...
                  >
                    ...
                    {post.replyTo && (
                      <div
                        className={`mx-3 mt-2 mb-1 p-2 rounded-lg text-xs min-w-0 ${
                          isMe
                    ...
                    {/* Image Attachment: strictly constrained to container */}
                    {post.fileType === 'image' && post.fileUrl && (
                      <div className="overflow-hidden bg-black/10 dark:bg-black/30 flex items-center justify-center w-full max-w-full">
                        <img
                          src={post.fileUrl}
                          alt="Shared attachment"
                          className="max-h-72 sm:max-h-96 w-full max-w-full object-contain block mx-auto"
                          loading="lazy"
                        />
                      </div>
                    )}
                    ...
                    {post.fileType === 'video' && post.fileUrl && (
                      <div className="overflow-hidden bg-black flex items-center justify-center w-full max-w-full">
                        <video
                          src={post.fileUrl}
                          controls
                          className="max-h-72 sm:max-h-96 w-full max-w-full object-contain block"
                        />
                      </div>
                    )}
                    ...
                    {/* Text Content */}
                    {isEditing ? (
                      <div className="p-3 flex flex-col gap-2 min-w-[180px]" onClick={(e) => e.stopPropagation()}>
                        <input
                          type="text"
                          value={editText}
                          onChange={(e) => setEditText(e.target.value)}
                          className="rounded-lg px-2.5 py-1.5 text-xs text-gray-900 bg-white border border-gray-300 focus:outline-none focus:ring-1 focus:ring-purple-500 w-full"
                          autoFocus
                        />
        ...
        {/* Input & Action Bar */}
        <div className="border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-[#0a0a14] px-3 sm:px-6 py-2.5 shrink-0 z-10 w-full max-w-full">
          {isMember ? (
            <div className="flex flex-col gap-2 max-w-4xl mx-auto w-full">
              ...
              {/* Message form */}
              <form onSubmit={handlePostSubmit} className="flex items-center gap-2 w-full">
                <input
                  ref={fileInputRef}
                  type="file"
                ...
                <button
                  ...
                  className="bg-purple-600 hover:bg-purple-700 text-white text-xs sm:text-sm font-semibold px-4 sm:px-5 py-2 rounded-full transition disabled:opacity-50 shrink-0 flex items-center gap-1.5"
                >
                  {posting ? (
                    <>
            ...
            <div className="py-2 text-center w-full">
              ...
              <button
                ...
                className="bg-purple-600 hover:bg-purple-700 text-white text-xs font-semibold px-5 py-2 rounded-full transition"
              >
                Join Now
              </button>
            </div>
