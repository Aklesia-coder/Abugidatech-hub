...
import {
  doc, getDoc, getDocs, updateDoc, collection, addDoc, query, where,
...
function Profile() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        if (userDoc.exists()) {
          ...
          setTelegramInput(data.telegram || '')
        }
      }
      setLoading(false)
    })
  ...
  const handleInfoSave = async () => {
    ...
    if (!fullNameInput.trim() || !usernameInput.trim()) {
      setInfoError('Name and username cannot be empty.')
      return
    }

    setSavingInfo(true)
    try {
      if (usernameInput.trim() !== profileData.username) {
        const usersRef = collection(db, 'users')
        const q = query(usersRef, where('username', '==', usernameInput.trim()))
      ...
      await updateProfile(user, { displayName: fullNameInput.trim() })
      ...
      console.error('Info save failed:', err)
      setInfoError('Something went wrong. Try again.')
    }
    setSavingInfo(false)
  }
    ...
    <div className="flex min-h-screen bg-white dark:bg-[#0a0a14] transition-colors duration-300">
      ...
      <div className="flex-1 px-6 md:px-12 py-10 pb-24 md:pb-10 text-gray-900 dark:text-white">
        ...
        <div className="max-w-md bg-purple-50 dark:bg-[#12101f] border border-purple-200 dark:border-purple-900 rounded-xl p-6 mb-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1">
              {!editingInfo ? (
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">Editing info below...</p>
              )}
            </div>
            <button
          ...
          {editingInfo && (
            <div className="flex flex-col gap-2 mb-5 bg-white dark:bg-[#0a0a14] rounded-lg p-3">
              {infoError && (
              ...
              <input
                ...
                className="border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-[#151225] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
              <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 mt-1">Username</label>
              <input
                type="text"
                value={usernameInput}
                ...
                className="border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-[#151225] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
              <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 mt-1">GitHub username</label>
              <input
                type="text"
                value={githubInput}
                ...
                className="border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-[#151225] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
              <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 mt-1">Telegram username</label>
              <input
                type="text"
                value={telegramInput}
              ...
              <button
                ...
                disabled={savingInfo}
                className="mt-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 rounded-lg text-sm transition disabled:opacity-50"
              >
          ...
          {(profileData?.github || profileData?.telegram) && !editingInfo && (
            <div className="flex gap-3 mb-5">
              {profileData?.github && (
                <a
                  ...
                  className="text-xs bg-gray-100 dark:bg-gray-800 px-3 py-1.5 rounded-full font-semibold hover:underline"
                >
                  🐙 GitHub
                </a>
              )}
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      <button
                        ...
                        className={'text-sm font-semibold flex items-center gap-1 ' + (hasLiked ? 'text-red-500' : 'text-gray-500 dark:text-gray-400')}
                      >
